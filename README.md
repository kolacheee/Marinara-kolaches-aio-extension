...
| **Right — Inspector** | Editable fields for whichever block you clicked. Save / Revert buttons in the footer (scroll position is preserved across saves). Switching to another block while dirty fires a Save / Revert / Stay confirmation. When the inspected block's content references one or more of the preset's variables, a collapsible **Preset variables** panel appears at the top — pick an option to substitute it live in the Simulated Prompt, and edit the option's value in place to PATCH it back to the preset. Field labels carry a `?` tooltip icon that explains what each field does on hover. |
