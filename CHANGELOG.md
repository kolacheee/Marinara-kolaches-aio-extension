...
- **🔍 Inspect → *Include* was quietly dropping your chat summary, lorebook entries, and tracker metadata.** The engine's dry-run endpoint treats those three as **explicit opt-ins** (its own route docs: *"Optional prompt injections are explicit opt-ins"*) even though real generation always includes them — and the extension never sent the flags. The capture came back with the summary set to `null`, the now-empty section was dropped by the assembler, and you got a prompt with no summary, no lorebook entries, and no trackers, with no error to hint at it. Inspect now opts into all three. (Reported by a user trying to read their summaries out of Inspect.)
- **The titlebar ✕ can no longer be pushed off a narrow window.** The bar had no `flex-wrap` and no shrink floor, so on a small screen its buttons squeezed until their labels wrapped under their icons, the title broke onto two lines, and the bar grew past the shell — carrying the close button out of reach. (Adding the *Summaries* button made a fifth control and tipped it over.) The title now ellipsizes instead of wrapping, controls never shrink, the bar can't outgrow its shell, and below 768px the buttons drop to emoji-only — their labels live on as tooltips. Verified down to 320px with an oversized font: the ✕ stays on screen, hit-testable, and closes the console.
- **Softened the "exact prompt" wording.** Even with those flags a dry run isn't byte-identical to a live send — it runs no agents or tools and passes no chat embedding, so semantically-recalled lorebook entries can still differ. The UI now says *resolved* prompt rather than *exact*.
...
- **Link *or embed* lorebooks on a character.** The character editor's Lorebook section links standalone lorebooks (auto-activation) and can now **Embed** one into the card — baking its entries into `data.character_book` so the lorebook travels with the card on export/sharing (linked lorebooks alone are dropped on export). Embedding sets the card's forward pointer + the lorebook's back-pointer so the engine keeps the embedded copy synced; Un-embed removes the baked-in copy but keeps the link; unlinking an embedded lorebook clears the embedded copy too. One embedded book per card.
- **Refresh reminder.** A `?` next to the titlebar title notes that edits save to Marinara immediately, but its own editors may keep showing the old version until you refresh the page (Ctrl+Shift+R) — the extension writes straight to the API and can't invalidate Marinara's in-memory cache.
