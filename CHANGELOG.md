...
- **Full character editor.** The character editor is now the complete V2 card in collapsible sections — *Metadata*, *Card* (now with **Backstory** and **Appearance**), *Dialogue & greetings* (incl. an add/remove alternate-greetings list), *Colors*, *Stats (RPG)* (editable attributes + pools), and *Advanced* (system prompt, post-history instructions, depth prompt) — with Name + Title always on top. Fields under the card's `extensions` (backstory, appearance, talkativeness, colors, stats, depth prompt) are mapped automatically, and everything the editor doesn't show (embedded lorebook, avatar crop, tracker colors, …) is preserved on save. Heavy text fields auto-grow to ~5 lines then offer an Expand toggle. *Lorebook*, *Sprites*, and *Gallery* appear as read-only summaries (they're files/embedded data managed in Marinara).
